export function spaceShipMove(state, deltaTime, THREE) {
  const targetSpaceShipPosition = new THREE.Vector3(
    state.spaceship.position.x,
    state.spaceship.position.y,
    state.spaceship.position.z
  );
  ...
  state.spaceship.position.lerp(
    targetSpaceShipPosition,
    state.lerpSpaceshipSpeed
  );
}
